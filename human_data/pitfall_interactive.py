"""
Playing PitFall-Atari2600 with keyboard and record actions with pickle
...
class Interactive(abc.ABC):
    ...
    """
    def __init__(self, env, sync=True, tps=60, aspect_ratio=None, record=False):
        self.actions = []
        self.play_num = 0
        obs = env.reset()
    ...
    def _update(self, dt):
        ...
        while self._sim_time < self._current_time:
            ...
            if not self._sync or act is not None:
                ...
                if self._record:
                    self.actions.append((act, _info['score']))
                #print("lives: ", _info['lives'])
                ...
                if done:
                    # record all actions from last play
                    self.play_num += 1
                    print("total actions in last play: ", len(self.actions))
                    filename = 'human_demonstration_{}.pkl'.format(self.play_num)
                    outfile = open(filename, 'wb')
                    pickle.dump(self.actions, outfile)
                    outfile.close()
                    #print(self.actions)
...
class RetroInteractive(Interactive):
    ...
    """
    def __init__(self, game, state, scenario, record, logKeys):
        env = retro.make(game=game, state=state, scenario=scenario)
        self._buttons = env.buttons
        self._logKeys = logKeys
        super().__init__(env=env, sync=False, tps=60, aspect_ratio=4/3, record=record)
    ...
    def keys_to_act(self, keys):
        ...
        act = [inputs[b] for b in self._buttons]
        if self._logKeys:
            print(keys)
            print(self._buttons)
            print(act)
...
def main():
    ...
    parser.add_argument('--game', default='Pitfall-Atari2600')
    parser.add_argument('--state', default=retro.State.DEFAULT)
    parser.add_argument('--scenario', default=None)
    parser.add_argument('--record', default=False)
    parser.add_argument('--logKeys', default=False)
    args = parser.parse_args()

    ia = RetroInteractive(game=args.game, state=args.state, scenario=args.scenario, record=args.record, logKeys=args.logKeys)
    ia.run()
